...
class TestNordPoolDTO:
    def test_parse_raw_today(self):
        ...
        dto.set_from_state(MockState({
            "raw_today": [
                {"start": datetime(2024, 1, 1, 0), "end": datetime(2024, 1, 1, 1), "value": 0.5},
                {"start": datetime(2024, 1, 1, 1), "end": datetime(2024, 1, 1, 2), "value": 0.7},
            ],
            "raw_tomorrow": [],
            "currency": "SEK",
        }))
    ...
    def test_parse_with_tomorrow(self):
        ...
        dto.set_from_state(MockState({
            "raw_today": [
                {"start": datetime(2024, 1, 1, i), "value": 1.0}
                for i in range(24)
            ],
            "raw_tomorrow": [
                {"start": datetime(2024, 1, 2, i), "value": 2.0}
                for i in range(24)
            ],
            "currency": "SEK",
        }))
